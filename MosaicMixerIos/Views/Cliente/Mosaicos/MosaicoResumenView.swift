...
struct MosaicoResumenView: View {
    ...
    @State private var navigateToCarrito = false  // Estado para la navegación
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    Button(action: {
                        if let metros = Double(metrosCuadrados), metros > 0 {
                            let item = CarritoItem(
                                mosaico: mosaico,
                                colores: selectedColors,
                                metrosCuadrados: Double(metrosCuadrados) ?? 0
                            )
                            carritoManager.agregarItem(item)
                            navigateToCarrito = true // Navegar al carrito
                        }
                    }) {
            ...
            // Navigation link para redirigir al carrito
            NavigationLink(destination: CarritoView(), isActive: $navigateToCarrito) {
                EmptyView()
            }
